...
*/
	...
	className=option("className"), 
		...
		el.setAttribute("r", String(roundTo(Math.max(0, Math.abs(circle.r*100.0)-width/2))));
		if(width != null) {
		...
		if(circle.a != 0) {
			el.setAttribute("transform", `rotate(${roundTo(circle.a)},${roundTo(circle.x*100)},${roundTo(circle.y*100)})`);
		}
		if(info != null) {
			el.setAttribute("data-info", JSON.stringify(info));
		}
			...
			else 
			{
				if(circle.r > minradius)
				{
					...
					ringpropsize = inputs["ringpropsizeslider"], */
						...
						curupdate.schedule(createSVGCircle, newgroup, new Circle(0, 0, circle.r, circle.a), 
							{
								width: strokewidth, 
								edgehue: calchue(circle, gapdepth, recdepth), 
								edgesat: strokesat, 
								edgeval: calcluma(circle, gapdepth, recdepth) + strokelum,
...
{
	/* this is the main entrypoint for initializing our input controls */
	...
	/* create clear buttons that appear when each clearable setting's input is not at the default value. */
...
addScript("https://unpkg.com/split-grid@1.0.9/dist/split-grid.js").then((evt,url,scriptel)=>{ 
	window.Split({
		columnGutters: [{
				track: 1,
...
addScript("https://cdn.jsdelivr.net/npm/svg-pan-zoom-container@0.2.7").then((evt, url, scriptel)=>{
...
(()=>{
	...
	document.addEventListener("readyStateChange", (evt)=>{
		...
		if(document.readyState == "complete" && !setupcalledonce) {
